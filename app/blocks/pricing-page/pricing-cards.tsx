    ...
    <section className={[styles.section, className].filter(Boolean).join(" ")}>
      <div className={styles.inner}>
        ...
        <div className={styles.grid}>
          {plans.map(p => (
            <div key={p.name} className={[styles.card, p.featured ? styles.featured : ""].join(" ")}>
              ...
              <Link to={p.link} className={[styles.btn, p.featured ? styles.btnPrimary : styles.btnOutline].join(" ")}>{p.cta}</Link>
            </div>
          ))}
        </div>
      </div>
